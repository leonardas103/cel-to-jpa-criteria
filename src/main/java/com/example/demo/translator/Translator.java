...
public class Translator {
    private static final Logger logger = LoggerFactory.getLogger(Translator2.class);
    ...
    private static <T> Predicate processFunctionCall(CelExpr.CelCall callExpr, Root<T> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
        ...
        switch (operator) {
            ...
            case "_!=_":
                Path<?> resolvedFieldPath = resolveFieldPath(arguments.get(0), root, cb, query);
            ...
            case "@in":
                ...
                return inPredicate;
            case "startsWith":
                return cb.like(resolveFieldPath(target.orElseThrow(), root, cb, query), arguments.get(0).constant().stringValue() +"%");
            case "contains":
                return cb.like(resolveFieldPath(target.orElseThrow(), root, cb, query), "%" + arguments.get(0).constant().stringValue() +"%");
            default:
